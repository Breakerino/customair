...
class X_Offcanvas_Nestable extends \Bricks\Element {
	...
	public function get_label() {
		return esc_html__( 'OffCanvas', 'extras' );
	}
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'x-offcanvas', BRICKSEXTRAS_URL . 'components/assets/js/offcanvas.js', '', '1.0.2', true );
	...
	public function render() {
		...
		$move_focus = isset( $this->settings['move_focus'] ) ? esc_attr__( $this->settings['move_focus'] ) : 'content';
		$focus_selector = isset( $this->settings['focus_selector'] ) ? esc_attr__( $this->settings['focus_selector'] ) : 'a';
		...
		if ( isset( $this->settings['force_second_close'] ) && 'true' === $this->settings['force_second_close'] ) {
			$offcanvas_config += [ 'secondClose' => $this->settings['second_offcanvas_selector'] ];
		}
