...
class X_Read_More_Less extends \Bricks\Element {
  ...
  public function enqueue_scripts() {
	  wp_enqueue_script( 'x-read-more-less', BRICKSEXTRAS_URL . 'components/assets/js/readmoreless.js', '', '1.0.4', true );
  }
  ...
  public function render() {
    ...
    if ( isset( $this->settings['moreText'] ) ) {
      $config += [ 'moreText' => esc_html( $this->settings['moreText'] )];
    }

    if ( isset( $this->settings['lessText'] ) ) {
      $config += [ 'lessText' => esc_html( $this->settings['lessText'] )];
    }

    if ( isset( $this->settings['moreAria'] ) ) {
      $config += [ 'moreAria' => esc_html( $this->settings['moreAria'] )];
    }

    if ( isset( $this->settings['lessAria'] ) ) {
      $config += [ 'lessAria' => esc_html( $this->settings['lessAria'] )];
    }
