...
class X_Pro_Slider extends \Bricks\Element {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'x-slider', BRICKSEXTRAS_URL . 'components/assets/js/proslider.js', '', '1.0.6', true );
	...
	public function set_control_groups() {
		...
		$this->control_groups['dynamicStyles'] = [
			'title' => esc_html__( 'Dynamic slide styles', 'bricks' ),
		];
	...
	public function set_controls() {
        ...
        $this->controls['perPage'] = [
            ...
            'css'   => [
				[
					...
					'selector' => '.splide__track_builder',
				],
			],
		];
        ...
        $this->controls['fixedWidth'] = [
            ...
            'css'   => [
				[
					'property' => 'width',
					'selector' => '.splide__track_builder .x-slider_slide',
				],
			],
		];
        ...
        $this->controls['autoHeight'] = [
			...
			'css'   => [
				[
					...
					'selector' => '.x-slider_builder .x-slider_slide',
					'value' => 'auto'
				],
			],
		];
		...
		*/
		...
		$this->controls['trackOverflow'] = [
			...
			'info' => esc_html__( "If changing the overflow, set overflow hidden on a container to prevent the slider from overflowing the page", 'bricks' ),
			'css'   => [
				[
					'property' => 'overflow',
					'selector' => '.splide__track',
				...
				[
					'property' => 'overflow',
					'selector' => '.x-slider_builder .splide__list',
				],
			],
		  ];
		...
		$this->controls['omitEnd'] = [
			'group'    => 'options',
			'label'    => esc_html__( 'Omit end dots', 'bricks' ),
			'type'     => 'checkbox',
		...
		$this->controls['syncSelector'] = [
			'tab' => 'content',
			'label' => esc_html__( 'Selector of the other slider to control', 'bricks' ),
	...
	public function render() {
		...
		$splide_options['rawConfig']['i18n'] = [
			'carousel' => !empty( $settings['i18nCarousel'] ) ? $settings['i18nCarousel'] : 'carousel'
		];

		if ( !empty( $settings['i18nPrev'] ) ) {
			$splide_options['rawConfig']['i18n'] += [ 'prev' => $settings['i18nPrev'] ];
		}

		if ( !empty( $settings['i18nNext'] ) ) {
			$splide_options['rawConfig']['i18n'] += [ 'next' => $settings['i18nNext'] ];
		}

		if ( !empty( $settings['i18nSlideX'] ) ) {
			$splide_options['rawConfig']['i18n'] += [ 'slideX' => $settings['i18nSlideX'] ];
		}

		if ( !empty( $settings['i18nPageX'] ) ) {
			$splide_options['rawConfig']['i18n'] += [ 'pageX' => $settings['i18nPageX'] ];
		}

		if ( !empty( $settings['i18nSelect'] ) ) {
			$splide_options['rawConfig']['i18n'] += [ 'select' => $settings['i18nSelect'] ];
		}

		if ( !empty( $settings['i18nSlide'] ) ) {
			$splide_options['rawConfig']['i18n'] += [ 'slide' => $settings['i18nSlide'] ];
		}

		if ( !empty( $settings['i18nSlideLabel'] ) ) {
			$splide_options['rawConfig']['i18n'] += [ 'slideLabel' => $settings['i18nSlideLabel'] ];
		}
		...
		$output .= !isset( $settings['galleryMode'] ) ? "</div>" : "";
