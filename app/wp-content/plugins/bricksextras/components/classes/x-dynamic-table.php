...
class X_Dynamic_Table extends \Bricks\Element {
		  ...
		  $this->controls['stackPadding'] = [
			...
			'css' => [
				[
				  ...
				  'selector' => '.x-dynamic-table_stacked td',
				]
			  ],
		  ];
  ...
  public function enqueue_scripts() {
	...
	wp_enqueue_script( 'x-table', BRICKSEXTRAS_URL . 'components/assets/js/dynamictable.js', '', '1.0.7', true );
  ...
  public function render() {
		...
		$config = [
			...
			'noRecordsFound' => isset( $settings['noRecordsFound'] ) ? $settings['noRecordsFound'] : 'No matching records found',
		];
		...
		if ( !isset( $settings['maybe_pagination'] ) || 'true' === $settings['maybe_pagination'] ) {
			$config += [
				...
				'previous' => isset( $settings['previous_text'] ) ? $settings['previous_text'] : 'Previous',
				'next' => isset( $settings['next_text'] ) ? $settings['next_text'] : 'Next',
				'showing' => isset( $settings['showing_text'] ) ? $settings['showing_text'] : 'Showing',
				'results' => isset( $settings['results_text'] ) ? $settings['results_text'] : 'Results',
				...
				'to' => isset( $settings['to_text'] ) ? $settings['to_text'] : 'to',
				'of' => isset( $settings['of_text'] ) ? $settings['of_text'] : 'of',
		...
		if ( 'dynamic' === $maybeDynamic ) {
			...
			if ( $content_items ) {			
				...
				foreach ( $content_items as $index => $content_item ) {
					...
					$header_title = ! empty( $content_item['title'] ) ? $content_item['title'] : '';

					$content_widths = ! empty( $content_item['width'] ) ? $content_item['width'] : null;

					echo '<th class="label" data-x-column="' . $content_widths . '">' . $header_title . '</th>';
