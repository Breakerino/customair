...
class X_Toggle_Switch extends \Bricks\Element {
  ...
  public function enqueue_scripts() {
    wp_enqueue_script( 'x-toggle-switch', BRICKSEXTRAS_URL . 'components/assets/js/toggleswitch.js', '', '1.0.2', true );
  }
  ...
  public function render() {
    ...
    $leftLabel = isset( $this->settings['leftLabel'] ) ? $this->settings['leftLabel'] : 'Monthly';
    $rightLabel = isset( $this->settings['rightLabel'] ) ? $this->settings['rightLabel'] : 'Yearly';
    $screenReaderText = isset( $this->settings['screenReaderText'] ) ? $this->settings['screenReaderText'] : 'Switch pricing';
    $ariaLabel = isset( $this->settings['ariaLabel'] ) ? $this->settings['ariaLabel'] : 'Switch content';
    ...
    if ('double' === $type ) {
      ...
      echo 
           '<label class="x-toggle-switch_switch">
              <div class="screen-reader-text">' . $screenReaderText . '</div>
        ...
        $this->set_attribute( "x-toggle-switch_label-0", 'tabindex', '0' );
          ...
          foreach ( $this->settings['labels'] as $index => $label ) {
            ...
            if ( ! empty( $label['title'] ) ) {
              ...
              echo "<button {$this->render_attributes( "x-toggle-switch_label-$index" )}><span>{$label['title']}</span></button>";
      ...
      echo '</div><div class="x-toggle-switch_multiple-slider"></div>';
