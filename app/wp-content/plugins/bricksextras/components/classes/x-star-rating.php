...
class X_Star_Rating extends \Bricks\Element {
  ...
  public function render() {
    ...
    $ariaLabel = "Rating: " . $starRating . " out of " . $totalStars . " stars";
	...
	if ( is_numeric( $starRating ) && is_numeric( $totalStars ) ) {
		...
		echo $totalStars >= $starRating ? str_repeat( $markedIcon, $starRating ) : str_repeat( $markedIcon, $totalStars );

		echo ( $starRating * 2 ) % 2 != 0 ? $halfmarkedIcon : '';

		if ( $totalStars - $starRating > 0 ) {
			echo str_repeat( $icon, $totalStars - $starRating );
		} 
	}
